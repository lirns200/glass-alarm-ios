        ...
        ZStack {
            ...
            VStack(spacing: 30) {
                HStack {
                    ...
                    Spacer()
                    if vm.combo > 1 {
                        Text("COMBO x\(vm.combo)").font(.headline.bold()).foregroundStyle(.orange).padding(8).background(Color.orange.opacity(0.1), in: Capsule())
                    }
                }
        ...
        ZStack {
            ...
            VStack(spacing: 24) {
                ...
                Button { vm.reset() } label: {
                    Text("TRY AGAIN").font(.headline.bold()).foregroundStyle(.white).padding(.horizontal, 40).padding(.vertical, 16).background(Color.blue, in: Capsule())
                }
            }
        }
    }
        ...
        ShapePreview(shape: shape, scale: isDragging ? 1.0 : 0.6)
            ...
            .gesture(
                DragGesture(coordinateSpace: .global)
                    ...
                    .onEnded { value in
                        // Dummy drop logic for testing
                        if abs(offset.height) > 100 {
                            // Find a valid spot (first available for test)
                            onDrop(0, 0) 
                        }
